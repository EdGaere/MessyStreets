"""
model.py: wrapper for geopy/Nominatim geolocation/geocoding service

NOTES
* Nominatim does not require an API key
...
class Model(ModelBase):
    ...
    def __init__(self
        ...
        self.geolocator = Nominatim(user_agent="Edward Gaere's PhD / Paper2",  timeout=30, ssl_context=ctx)
    ...
    @adisk_cache('~/data/cache/nominatim')
    async def _call_api(self, input_sequence : str) -> Any:
        # NOTE: set addressdetails to get the address details for Nominatim
        if self.debug:
            model_logger.debug(f"Sending request to Nominatim...")
        return self.rate_limited_geocoder(input_sequence, addressdetails=True)
...
if __name__ == "__main__":
    ...
    def main(config : str, input_sequence : str, clear_cache : bool = False, no_cache : bool = False, debug : bool = False, debug2 : bool = False):
        async_run(amain(config, input_sequence, clear_cache=clear_cache, no_cache=no_cache, debug=debug, debug2=debug2))

    async def amain(config : str, input_sequence : str,  clear_cache : bool = False, no_cache : bool = False, debug : bool = False, debug2 : bool = False):
        """
        ...
        if no_cache:
            ...
            result = model.rate_limited_geocoder(input_sequence, addressdetails=True)
            if result is None:
                print("no prediction")
                return
            else:
