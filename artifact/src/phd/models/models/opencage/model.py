"""
model.py: wrapper for geopy/OpenCage geolocation/geocoding service

SOURCE
* https://opencagedata.com/dashboard#geocoding

* API_KEY : <Code Book:OpenCage>
...
class Model(ModelBase):
    ...
    def __init__(self
        ...
        # get API KEY
        if "OPENCAGE_API_KEY" not in environ:
            raise RuntimeError(f"OPENCAGE_API_KEY not specified")
        
        api_key = environ['OPENCAGE_API_KEY']
        model_logger.debug(f"api_key : {api_key}")

        # create geolocator instance with Rate Limiter
        ctx = ssl.create_default_context(cafile=certifi.where())
        self.geolocator = OpenCage(user_agent="Edward Gaere's PhD / Paper2"
                                 , api_key=api_key
                                 , timeout=30
                                 , ssl_context=ctx
                                 )
    ...
    @adisk_cache('~/data/cache/opencage')
