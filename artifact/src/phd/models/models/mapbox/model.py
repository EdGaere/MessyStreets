"""
model.py: wrapper for geopy/MapBox geolocation/geocoding service

SOURCE
* https://account.mapbox.com/access-tokens/

* API_KEY : <Code Book:mapbox>
...
class Model(ModelBase):
    ...
    def __init__(self
        ...
        # get API KEY
        if "MAPBOX_API_KEY" not in environ:
            raise RuntimeError(f"MAPBOX_API_KEY not specified")
        
        api_key = environ['MAPBOX_API_KEY']
        model_logger.debug(f"api_key : {api_key}")

        # create geolocator instance with Rate Limiter
        ctx = ssl.create_default_context(cafile=certifi.where())
        self.geolocator = MapBox(user_agent="Edward Gaere's PhD / Paper2"
                                 , api_key=api_key
                                 , timeout=30
                                 , ssl_context=ctx
                                 )
