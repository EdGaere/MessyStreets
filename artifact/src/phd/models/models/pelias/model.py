"""
model.py: wrapper for geopy/Pelias geolocation/geocoding service

NOTES
* We use the offical Pelias implementation from geocode.earth, not a self-hosted version => reproducibility, configuration bias

* API KEY : <Code Book:geocode.earth>

SOURCE
https://github.com/pelias/documentation
Pelias is a search engine for places worldwide, powered by open data. 
It turns addresses and place names into geographic coordinates, and turns geographic coordinates into places and addresses.

2014 — Created at Mapzen, a mapping company founded by Samsung that was building open-source mapping tools
2017 — Mapzen shut down (Samsung pulled funding)
2018 — Pelias moved to the Linux Foundation as an open-source project
...
class Model(ModelBase):
    ...
    def __init__(self
        ...
        # get API KEY
        if "GEOCODE_EARTH_API_KEY" not in environ:
            raise RuntimeError(f"GEOCODE_EARTH_API_KEY not specified")
        
        api_key = environ['GEOCODE_EARTH_API_KEY']
        model_logger.debug(f"api_key : {api_key}")

        # create geolocator instance with Rate Limiter
        ctx = ssl.create_default_context(cafile=certifi.where())
        self.geolocator = Pelias(domain="api.geocode.earth"
                                 , api_key=api_key
                                 , user_agent="Edward Gaere's PhD / Paper2"
                                 , timeout=30
                                 ,  ssl_context=ctx)
        ...
        if location is not None:
            ...
            if self.component == "geohash":
                ...
                try:
                    ...
                    model_logger.error(f"{type(e)} | {e} | returned location has no 'address' component | {location.raw.keys()}")
                    exit(1)
