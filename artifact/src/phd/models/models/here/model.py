"""
model.py: wrapper for geopy/Here geolocation/geocoding service

SOURCE
* https://platform.here.com/portal/

* API_KEY : <Code Book:here>
...
class Model(ModelBase):
    ...
    def __init__(self
        ...
        # get API KEY
        if "HERE_API_KEY" not in environ:
            raise RuntimeError(f"HERE_API_KEY not specified")
        
        api_key = environ['HERE_API_KEY']
        model_logger.debug(f"api_key : {api_key}")

        # create geolocator instance with Rate Limiter
        ctx = ssl.create_default_context(cafile=certifi.where())
        self.geolocator = HereV7(user_agent="Edward Gaere's PhD / Paper2"
                                 , apikey=api_key
                                 , timeout=30
                                 , ssl_context=ctx
                                 )
