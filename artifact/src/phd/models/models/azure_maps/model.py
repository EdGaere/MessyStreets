"""
model.py: wrapper for geopy/Azure Maps geolocation/geocoding service

SOURCE
* portal.azure.com

* API_KEY : <Code Book:Windows Azure (Personal)>
...
class Model(ModelBase):
    ...
    def __init__(self
        ...
        # get API KEY
        if "AZURE_MAPS_API_KEY" not in environ:
            raise RuntimeError(f"AZURE_MAPS_API_KEY not specified")
        
        api_key = environ['AZURE_MAPS_API_KEY']
        model_logger.debug(f"api_key : {api_key}")

        # create geolocator instance with Rate Limiter
        ctx = ssl.create_default_context(cafile=certifi.where())
        self.geolocator = AzureMaps(user_agent="Edward Gaere's PhD / Paper2"
    ...
    @adisk_cache('~/data/cache/azure_maps')
