"""
model.py: wrapper for geopy/geoapify Maps geolocation/geocoding service

SOURCE
* portal.azure.com

* API_KEY : <Code Book:geoapify>
...
python model.py geohash1 "The Book Club 100-106 Leonard St Shoreditch London EC2A 4RH, United Kingdom"
...
class Model(ModelBase):
    ...
    def __init__(self
        ...
        # get API KEY
        if "GEOAPIFY_MAPS_API_KEY" not in environ:
            raise RuntimeError(f"GEOAPIFY_MAPS_API_KEY not specified")
        
        self.api_key = environ['GEOAPIFY_MAPS_API_KEY']
        model_logger.debug(f"api_key : {self.api_key}")

        # create geolocator instance with Rate Limiter
        self.ssl_context = ssl.create_default_context(cafile=certifi.where())

        self.client = Client(base_url="https://api.geoapify.com/v1", timeout=30.0)


    @adisk_cache('~/data/cache/geoapify')
    async def _call_api(self, address: str) -> Dict:
        model_logger.debug(f"API call | {address}")
