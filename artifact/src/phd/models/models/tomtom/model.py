"""
model.py: wrapper for geopy/TomTom geolocation/geocoding service
...
class Model(ModelBase):
    ...
    def __init__(self
        ...
        # get API KEY
        if "TOMTOM_API_KEY" not in environ:
            raise RuntimeError(f"TOMTOM_API_KEY not specified")
        
        api_key = environ['TOMTOM_API_KEY']
        model_logger.debug(f"api_key : {api_key}")

        # create geolocator instance with Rate Limiter
        ctx = ssl.create_default_context(cafile=certifi.where())
        self.geolocator = TomTom(user_agent="Edward Gaere's PhD / Paper2"
                                 , api_key=api_key
                                 , timeout=30
                                 , ssl_context=ctx
                                 )
